...
const STATUS_STYLE: Record<string, { label: string; color: string; bg: string }> = {
  sold:      { label: 'Ativa',    color: AC2,       bg: 'rgba(168,85,247,.12)' },
  suspended: { label: 'Suspensa', color: '#fbbf24', bg: 'rgba(251,191,36,.12)' },
  removed:   { label: 'Removida', color: '#6b7280', bg: 'rgba(107,114,128,.12)' },
  available: { label: 'Livre',    color: '#34d399', bg: 'rgba(52,211,153,.12)' },
}
...
export default function ClientesPage() {
  ...
  async function loadClientProxies(clientId: string) {
    ...
    setExpanded(e => ({ ...e, [clientId]: Array.isArray(data) ? data : [] }))
  ...
  async function updateProxyStatus(clientId: string, proxyId: string, newStatus: 'sold' | 'suspended') {
    ...
    try {
      ...
      if (res.ok) loadClientProxies(clientId)
    } finally {
  ...
  function copyCredentials(px: Proxy) {
    navigator.clipboard.writeText(`${px.host}:${px.port}:${px.username}:${px.password}`).then(() => {
      setCopied(px.id)
                    ...
                    {isOpen && (
                      <div style={{ padding: '4px 20px 16px 62px', background: 'rgba(0,0,0,.15)' }}>
                        {isPxLoading ? (
                          ...
                          <div style={{ fontSize: 12, color: 'rgba(244,242,248,.3)', padding: '12px 0' }}>Nenhuma proxy registrada para este cliente.</div>
                        ) : (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, paddingTop: 8 }}>
                            {proxies.map(px => {
                              const st       = STATUS_STYLE[px.status] ?? STATUS_STYLE.available
                              ...
                              const isRemoved = px.status === 'removed'

                              return (
                                <div key={px.id} style={{ background: 'rgba(255,255,255,.03)', border: `1px solid ${isRemoved ? 'rgba(107,114,128,.15)' : 'rgba(255,255,255,.07)'}`, borderRadius: 12, padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 10, opacity: isRemoved ? .6 : 1 }}>

                                  {/* TOP */}
                                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                      <span style={{ width: 7, height: 7, borderRadius: '50%', background: st.color, boxShadow: isRemoved ? 'none' : `0 0 5px ${st.color}`, flexShrink: 0, display: 'inline-block' }} />
                                      <span style={{ fontSize: 13, fontWeight: 700, color: '#f4f2f8' }}>{px.label || 'Proxy'}</span>
                                      <span style={{ fontSize: 10.5, padding: '2px 7px', borderRadius: 5, background: st.bg, color: st.color, fontWeight: 700, letterSpacing: '.04em' }}>
                                        {st.label}
                                      </span>
                                    </div>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: 11, color: 'rgba(244,242,248,.4)' }}>
                                      {px.sold_at && <span>desde {fmtDate(px.sold_at)}</span>}
                                  ...
                                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10.5, color: 'rgba(244,242,248,.4)' }}>
                                      ...
                                      <span style={{ fontWeight: 600, color: isRemoved ? 'rgba(244,242,248,.3)' : barColor }}>{Number(px.used_gb).toFixed(2)} / {px.gb_limit} GB · {pct}%</span>
                                    </div>
                                    <div style={{ height: 5, borderRadius: 3, background: 'rgba(255,255,255,.07)', overflow: 'hidden' }}>
                                      <div style={{ height: '100%', width: `${pct}%`, background: isRemoved ? 'rgba(107,114,128,.4)' : barColor, borderRadius: 3, transition: 'width .4s' }} />
                                    </div>
                                  </div>
                                  ...
                                  {!isRemoved && (
                                    <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                                      ...
                                      >
                                        ...
                                        {check === 'ok' ? '✓ Online' : check === 'fail' ? '✗ Offline' : 'Verificar'}
                                      </button>

                                      {px.status === 'suspended' ? (
                                        <button
                                          onClick={() => updateProxyStatus(c.id, px.id, 'sold')}
                                          disabled={busy}
                                        ...
                                        <button
                                          onClick={() => updateProxyStatus(c.id, px.id, 'suspended')}
                                          disabled={busy}
